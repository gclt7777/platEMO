...
4. The *variable clustering* and *correlation analysis* phases now scale their sampling automatically based on the remaining evaluation budget. They will stop early rather than exceeding `maxFE`, so the convergence plot will no longer overshoot the user-specified evaluation limit.
