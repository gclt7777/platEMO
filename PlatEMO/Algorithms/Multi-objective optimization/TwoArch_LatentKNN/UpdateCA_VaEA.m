...
% 2) 临界层按 “fit = sum(归一化目标)” 升序截断
    ...
    [scoreAll, fitAll, distAll] = VaEAUtils.ConvergenceScore(ObjsN);
    keep = FrontNo < MaxFNo;
    CA   = Union(keep);
    ...
    if remain > 0
        ...
        metrics = [fitAll(lastIdx), distAll(lastIdx), scoreAll(lastIdx)];
        [~, order] = sortrows(metrics, [1 2 3]);
    ...
    elseif length(CA) > N_CA
        ...
        [scoreCA, fitCA, distCA] = VaEAUtils.ConvergenceScore(ObjsN_CA);
        metrics = [fitCA, distCA, scoreCA];
        [~, ord] = sortrows(metrics, [1 2 3]);
