...
methods(Static)
    ...
    function CA = RefineWithSeeds(CA, DA, zmin, zmax)
        if isempty(CA) || isempty(DA)
            return;
        end

        [ObjsN_CA, ~, ~] = VaEAUtils.NormalizeObjs(CA.objs, zmin, zmax);
        [scoreCA, ~, ~]   = VaEAUtils.ConvergenceScore(ObjsN_CA);
        scoreCA = scoreCA(:);

        [ObjsN_DA, ~, ~] = VaEAUtils.NormalizeObjs(DA.objs, zmin, zmax);
        seedIdx = VaEAUtils.ExtremeSeeds(ObjsN_DA, min(size(ObjsN_DA,1), numel(CA)));
        seedIdx = seedIdx(:)';

        for idx = seedIdx
            [seedScoreVec, ~, ~] = VaEAUtils.ConvergenceScore(ObjsN_DA(idx,:));
            seedScore = seedScoreVec(1);
            [worstScore, worstId] = max(scoreCA);
            if seedScore < worstScore
                CA(worstId) = DA(idx);
                ObjsN_CA(worstId,:) = ObjsN_DA(idx,:);
                scoreCA(worstId) = seedScore;
            end
        end
    end
end
